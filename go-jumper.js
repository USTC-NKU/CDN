...
async function checkLink(){
    ...
    for(var i=0;i<link.length;i++){
        ...
        if(!await checkLocalSite(link[i].href)&&link[i].type!="fancyimg"){
            link[i].href = "https://jump.starysky.top/#"+window.btoa(link[i].href)
            //console.log("edit.")
        }
    }
}
async function checkLocalSite(url){
    try{
        ...
        //console.log(domain)
        if(domain.check==="starysky.top" || domain.original[0].split('/')[0]==="localhost:4000")return true;//将domain.check修改为根或二级域名，domain.original[0].split('/')[0]修改为本地测试页面
